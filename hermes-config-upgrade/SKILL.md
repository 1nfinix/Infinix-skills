...
description: "Hermes 配置审计、升级迁移与自适应优化。检测当前配置完整性、适配新版本、修复已知断裂模式。neat-freak v3 第零步检测到版本变化时自动触发子流程。"
version: 2.1.0
platforms: [linux]
metadata:
  hermes:
...
- Hermes 版本升级后
- neat-freak v3 第零步检测到版本变化 → 自动触发子流程
- 发现模型/API/服务异常 → 排查配置
...
被 neat-freak v3 第零步调用时，只执行以下 5 项快速检查：
...
grep -A1 "provider:" ~/.hermes/config.yaml | grep -v "^--$" | sort | uniq
...
| 诊断发现 | 修复命令 |
|---------|---------|
| `providers: {}` 为空但 fallback 引用某 provider | `hermes config set providers.<name>.api_key '$YOUR_API_KEY'` |
| fallback_model 指向已下架模型 | `hermes config set fallback_model.model '<可用模型>'` |
| auxiliary.vision 用非视觉模型 | `hermes config set auxiliary.vision.model <视觉模型>` |
| web.backend 与 web.search_backend 不一致 | `hermes config set web.search_backend <backend>` |
...
for p, n in cnt.most_common():
    pct = n / len(aux) * 100 if aux else 0
...
- [ ] max_concurrent_children ≤ 当前配置限制
...
**检查项：**
- [ ] TTS provider 对应服务可达（检查 config 中配置的实际 provider 健康状态）
- [ ] 备用 provider 配置完整
...
**检查项：**
- [ ] search_backend 对应的 API Key 有效且格式正确
- [ ] 确认 `.env` 中对应环境变量存在
...
| 断裂模式 | 症状 | 自动修复 |
|---------|------|---------|
| `providers: {}` 为空但 fallback 引用某 provider | errors.log: `unknown provider` | `hermes config set providers.<name>.api_key '$YOUR_API_KEY'` |
| `fallback_model.model` 指向已下架模型 | HTTP 404 | `hermes config set fallback_model.model '<可用模型>'` |
| auxiliary.vision 用非视觉模型 | vision_analyze 返回错误 | 替换为视觉模型 |
| `api_key: ''` 空字符串 | HTTP 401 | 替换为 `${ENV_VAR_NAME}` 格式 |
| `hermes config set` 把 list 值序列化成 JSON string | fallback_providers 变成字符串 | 用 python3 + yaml 库 unwrap |
...
| TTS provider 对应的服务不可达 | text_to_speech 失败 | 检查对应服务状态或切换 provider |
...
不推荐具体模型/厂商，根据以下原则自行判断：

| 原则 | 说明 |
|------|------|
| **厂商隔离** | 主模型、fallback[0]、delegation 三者用不同厂商 |
| **角色匹配** | vision 用视觉模型，web_extract 用快速模型，TTS 用音频模型 |
| **成本分层** | 高频低风险角色（压缩/摘要/web_extract）用便宜模型 |
| **辅助模型分散** | 10+ 辅助角色避免全部指向同一厂商 |
| **磁盘适配** | 磁盘紧张时降低 checkpoints.max_disk_mb |
| **长会话支持** | memory 上限 ≥ 8000，agent.max_turns ≥ 120 |
| **TTS 选型** | 优先本地服务（低延迟免费），备选云端 API |
...
- 其余 N 个: <provider> / <model>
- 厂商集中度: <provider> <percent>% 🟡
